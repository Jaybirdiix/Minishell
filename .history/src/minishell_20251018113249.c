...
/* After expansion, restore placeholder (UNIT SEPARATOR 0x1F) back to '$' */
...
/* Adds quoting with minimal changes to the rest of the code:
   ...
   - Double quotes support backslash escapes (\n, \t, \", \\); expansion now deferred.
   - Single quotes are literal; we mark '$' as placeholder 0x1F to prevent expansion later.
   - Operators remain separate tokens: | & < > >> << && ||.
   - IMPORTANT: We NO LONGER call expand_vars() here. We defer to execution time.
*/
    ...
    while (*p) {
        ...
        while (*p && !isspace((unsigned char)*p) && !strchr("|&<>", *p) && !is_op2(p[0], p[1])) {
            if (*p == '\'') { /* single-quoted: literal until next ' */
                p++; /* skip opening ' */
                while (*p && *p != '\'') {
                    /* mark '$' to prevent later expansion */
                    char c = *p++;
            ...
            if (*p == '\"') { /* double-quoted: allow escapes; expansion later */
                p++; /* skip opening " */
                ...
                /* append quoted chunk (no expansion yet) */
                if (dq) {
...
static void setup_redirs(const Redir *r) {
    int fd;
    if (r->in_file) {
        /* expand variables in redir filenames too */
...
static void exec_time_expand_argv(char **argv) {
    for (int i=0; argv[i]; ++i) {
        char *exp = expand_vars(argv[i]);   /* expands $VAR and $? */
