...
static int run_simple(struct Node *n) {
    ...
    // Execute-time argv expansion so $? reflects the most recent command
...
/* Pipe execution.
 * "Counts cmds in a pipe" (conceptually) by running left and right.
 * Returns the rightmost command's status, like a typical shell.
 */
...
/* Recursively run nodes; mirrors original SEP_AND/OR/BG logic:
