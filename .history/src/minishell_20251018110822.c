...
/* Expands $VAR and $?; no braces here. */
...
/* --------------------- Tokenizer -------------------------- */
/* Adds quoting with minimal changes to the rest of the code:
   - Words can be unquoted, "double-quoted", or 'single-quoted'.
   - Double quotes support backslash escapes (\n, \t, \", \\) and $-expansion.
   - Single quotes are literal (no escapes, no expansion).
   - Operators remain separate tokens: | & < > >> << && ||.
*/
...
/* helper: append one char to dynamic buffer */
...
static char **tokenize(const char *line, int *outc) {
    ...
    while (*p) {
        ...
        /* word (possibly quoted mix) */
        char *acc = NULL; size_t len = 0, cap = 0;

        while (*p && !isspace((unsigned char)*p) && !strchr("|&<>", *p) && !is_op2(p[0], p[1])) {
            if (*p == '\'') { /* single-quoted: literal until next ' */
                p++; /* skip opening ' */
                while (*p && *p != '\'') {
                    sb_putc(&acc, &len, &cap, *p++);
                }
                if (*p == '\'') p++; else { /* unclosed */
                    fprintf(stderr, "syntax error: unmatched '\\''\n");
                    free(acc);
                    /* drain rest of line to avoid infinite loop */
                    while (*p) p++;
                    goto finish_line;
                }
                continue;
            }
            if (*p == '"') { /* double-quoted: allow escapes and $-expansion later */
                p++; /* skip opening " */
                /* collect quoted chunk in a temp, then expand vars on that chunk */
                ...
                /* expand variables inside the double-quoted chunk */
                char *exp = expand_vars(dq ? dq : "");
                free(dq);
                /* append to accumulating word */
                for (char *q = exp; *q; q++) sb_putc(&acc, &len, &cap, *q);
                free(exp);
                continue;
            }

            /* unquoted char: may start a $VAR which expand_vars handles later.
               We accumulate a raw run (stopping at whitespace/op). */
            sb_putc(&acc, &len, &cap, *p++);
        }
        ...
        /* For unquoted portions we still need $-expansion overall.
           Since we may have mixed quoted (already expanded) + unquoted, just run expand_vars once on the full acc. */
        char *final = expand_vars(acc);
        free(acc);
        vec_push(&tok, final);
    }
...
/* ---- Parser (unchanged structure, honors same operators) ---- */
...
static void setup_redirs(const Redir *r) {
    ...
    if (r->heredoc_data) {
        ...
        fd = mkstemp(tmp);
        if (fd < 0) { perror("mkstemp"); _exit(127); }
        ssize_t w = write(fd, r->heredoc_data, (ssize_t)strlen(r->heredoc_data));
        (void)w;
        lseek(fd, 0, SEEK_SET);
        if (dup2(fd, STDIN_FILENO) < 0) { perror("dup2 heredoc"); _exit(127); }
        close(fd);
        unlink(tmp);
    }
    if (r->out_file) {
        int flags = O_WRONLY | O_CREAT | (r->out_append ? O_APPEND : O_TRUNC);
        fd = open(r->out_file, flags, 0644);
        if (fd < 0) { perror("open >"); _exit(127); }
...
static int run_node(struct Node *n) {
    if (!n) return 0;
    switch (n->type) {
        case NT_SIMPLE: return run_simple(n);
        case NT_PIPE:   return run_pipe(n);
        case NT_AND: {
            int ls = run_node(n->left); if (ls == 0) return run_node(n->right); else return ls;
        }
        case NT_OR: {
            int ls = run_node(n->left); if (ls != 0) return run_node(n->right); else return ls;
        }
        case NT_BG: {
            ...
            if (pid == 0) {
                ...
                return n->right ? run_node(n->right) : 0;
            }
        }
    }
    return 1;
